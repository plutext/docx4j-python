...
```bash
.venv-fork/bin/python -m pytest                    # everything, 1,219 tests
...
> **Not yet run: artefact 9, regenerated 2026-09-17** after CR-003 section 17.10: it now fills
> **three** line items, so the check is that Word shows three rows in the line-item table (it
> expands the repeat itself; the file carries one template item). Everything else in its row is
> unchanged, and no other artefact is affected.
>

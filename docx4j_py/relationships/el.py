"""``el`` for the http://schemas.openxmlformats.org/package/2006/relationships namespace.

Generated by ``codegen/generate_el.py`` from the ``XmlContext`` class metadata;
do not edit. The naming rules, the collisions and the class each name resolves
to are in ``codegen/el_tables/relationships.json``, which a human reviews.

    >>> from docx4j_py.relationships import el
    >>> el.Relationship()  # doctest: +SKIP

Every function takes the dataclass's own keyword arguments and passes them
straight through. A class that carries text (``w:t``, ``w:delText``,
``w:instrText``: anything whose metadata has a text value field) also takes the
text as the one positional argument, and sets ``xml:space="preserve"`` when the
text begins or ends with whitespace or holds two in a row, which is what
docx4j's ``t()`` does.
"""

from __future__ import annotations

from typing import Any

from docx4j_py.runtime import text_element

from docx4j_py.relationships import (
    Relationship as _Relationship,
    Relationships as _Relationships,
)

__NAMESPACE__ = "http://schemas.openxmlformats.org/package/2006/relationships"

def Relationship(**kwargs: Any) -> _Relationship:
    """``{http://schemas.openxmlformats.org/package/2006/relationships}Relationship`` -> :class:`docx4j_py.relationships.Relationship`."""
    return _Relationship(**kwargs)

def Relationships(**kwargs: Any) -> _Relationships:
    """``{http://schemas.openxmlformats.org/package/2006/relationships}Relationships`` -> :class:`docx4j_py.relationships.Relationships`."""
    return _Relationships(**kwargs)

#: Element qualified name -> the class ``el.<name>`` builds.
QNAME_TO_CLASS: dict[str, type] = {
    "{http://schemas.openxmlformats.org/package/2006/relationships}Relationship": _Relationship,
    "{http://schemas.openxmlformats.org/package/2006/relationships}Relationships": _Relationships,
}

#: Local element name -> the class ``el.<name>`` builds.
ELEMENTS: dict[str, type] = {
    "Relationship": _Relationship,
    "Relationships": _Relationships,
}

#: Class -> the one element qualified name it stands for, where there is one.
CLASS_TO_QNAME: dict[type, str] = {
    _Relationship: "{http://schemas.openxmlformats.org/package/2006/relationships}Relationship",
    _Relationships: "{http://schemas.openxmlformats.org/package/2006/relationships}Relationships",
}

__all__ = [
    "CLASS_TO_QNAME",
    "ELEMENTS",
    "QNAME_TO_CLASS",
    "Relationship",
    "Relationships",
]

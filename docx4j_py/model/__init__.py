...
and ``Font`` (Phase B) and the agent surface --- addresses, ``Outline``,
``find()``, ``describe()``, ``ChangeReport``, ``dry_run`` (Phase D);
:mod:`docx4j_py.model.sessions` holds ``DocumentSession``, the registry a server
keeps documents open in. Markdown and custom XML join them in later phases.
